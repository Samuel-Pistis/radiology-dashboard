    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-surface rounded-xl border border-surface-hover p-6 shadow-sm">
                ...
                <form onSubmit={handleAddModality} className="flex gap-3 mb-6">
                    <input
                        type="text"
                        value={newModalityName}
                        onChange={(e) => setNewModalityName(e.target.value)}
                        placeholder="New Modality Name..."
                        className="flex-1 bg-background border border-surface-hover rounded-lg px-4 py-2 text-text-primary focus:outline-none focus:ring-2 focus:ring-primary-500 placeholder-text-secondary transition-all"
                    />
                    <button
                        type="submit"
                        disabled={!newModalityName.trim()}
                        className="bg-primary-600 hover:bg-primary-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-5 py-2 rounded-lg font-medium flex items-center gap-2 transition-colors duration-200"
                    >
